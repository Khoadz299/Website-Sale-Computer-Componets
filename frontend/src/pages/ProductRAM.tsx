...
const ProductRAM = () => {
    ...
    (term: string) => {
      const filtered = [...products].filter(
        (product) =>
          ...
          product.capacity.toLowerCase().includes(term.toLowerCase()) ||
          product.bus_speed.toLowerCase().includes(term.toLowerCase()) ||
          product.capacity.toLowerCase().includes(term.toLowerCase()) ||
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch(`http://localhost:6060/product-ram`);
    ...
    const fetchImageProducts = async () => {
      try {
        const response = await fetch(
          `http://localhost:6060/list-image-product/type/RAM`
        );
  ...
  return (
    <div className="container mx-auto py-8 w-4/5">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold">Sản phẩm RAM</h1>
        <div>
          <input
            type="text"
            placeholder="Search..."
        ...
        {displayedProducts.map((product) => (
          <div key={product.id} className="border rounded p-4">
            ...
            <div className="">
              ...
              <p className="text-gray-600 mt-3">
                Tốc độ buss : {product.bus_speed}
              </p>
              <p className="text-gray-600 mt-3">
                Loại bộ nhớ : {product.model}
              </p>
