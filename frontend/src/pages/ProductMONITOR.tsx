...
const ProductMONITOR = () => {
    ...
    (term: string) => {
      const filtered = [...products].filter(
        (product) =>
          ...
          product.resolution.toLowerCase().includes(term.toLowerCase()) ||
          product.screen_size.toLowerCase().includes(term.toLowerCase()) ||
          product.brand.toLowerCase().includes(term.toLowerCase()) ||
          product.panel_type.toLowerCase().includes(term.toLowerCase()) ||
          product.response_time.toLowerCase().includes(term.toLowerCase())
      );
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch(`http://localhost:6060/product-monitor`);
    ...
    const fetchImageProducts = async () => {
      try {
        const response = await fetch(
          `http://localhost:6060/list-image-product/type/MONITOR`
        );
  ...
  return (
    <div className="container mx-auto py-8 w-4/5">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold">Sản phẩm màn hình</h1>
        <div>
          <input
            type="text"
            placeholder="Search..."
        ...
        {displayedProducts.map((product) => (
          <div key={product.id} className="border rounded p-4">
            <img
              src={product.url}
              alt={product.name}
              className="w-auto h-[200px] object-cover mb-4 mx-auto"
            />
            <h2 className="text-lg font-semibold">{product.name}</h2>
            <p className="text-gray-600 mt-4">Thương hiệu : {product.brand}</p>
            ...
            <p className="text-gray-600 mt-4">Tấm nền : {product.resolution}</p>
            <p className="text-gray-600 mt-4">
