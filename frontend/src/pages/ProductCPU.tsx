...
const ProductList = () => {
    ...
    (term: string) => {
      const filtered = [...products].filter(
        (product) =>
          ...
          product.socket.toLowerCase().includes(term.toLowerCase()) ||
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch(`http://localhost:6060/product-cpu`);
    ...
    const fetchImageProducts = async () => {
      try {
        const response = await fetch(
          `http://localhost:6060/list-image-product/type/CPU`
        );
  ...
  return (
    <div className="container mx-auto py-8 w-4/5">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold">Sản phẩm chip xử lý CPU</h1>
        <div>
          <input
            type="text"
            placeholder="Search..."
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {displayedProducts.map((product) => (
          <div key={product.id} className="border rounded p-4">
            ...
            <p className="text-gray-600 mt-3">Socket: {product.socket}</p>

            <p className="text-gray-600 mt-3">Phân loại: {product.model}</p>
